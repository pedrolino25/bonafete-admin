...
interface SpaceInfoSectionProps {
  ...
  conveniencesList: SpaceConvenienceListItemResponse[]
...
export default function SpaceInfoSection({
  ...
  const updateSpaceInfoMutation = useMutation({
    mutationFn: updateSpaceInfo,
    onSuccess: () => {
      refetch?.()
      toast({
        variant: 'success',
  ...
  const onSubmit = (values: SpaceInfoFormType) => {
    updateSpaceInfoMutation.mutate({
        ...
        <div className="flex justify-between items-center gap-4 max-sm:justify-end max-sm:items-start max-sm:pt-4 max-sm:w-full">
          ...
          <Button
            ...
            disabled={
              !isValid ||
              (completed && !isDirty) ||
              updateSpaceInfoMutation.isPending ||
              lastVerification !== getValues().title
            }
            loading={updateSpaceInfoMutation.isPending}
