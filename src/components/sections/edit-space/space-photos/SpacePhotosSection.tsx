...
import { SpaceInfo, updateSpacePhotos } from '@/services/api/spaces'
...
export default function SpacePhotosSection({
  ...
  const updateSpacePhotosMutation = useMutation({
    mutationFn: updateSpacePhotos,
    onSuccess: () => {
      setLoading(false)
      refetch?.()
      toast({
  ...
  const onSubmit = async (values: SpacePhotosFormType) => {
    ...
    updateSpacePhotosMutation.mutate({
      photos: pictures,
    ...
    >
      <div className="w-full border-b px-6 max-sm:px-4 pb-4 flex justify-between items-center max-sm:flex-col">
        ...
        <div className="flex justify-between items-center gap-4 max-sm:justify-end max-sm:items-start max-sm:pt-4 max-sm:w-full">
          {showUpdateOnboardingStatus && (
            <Button
              disabled={!isValid || updateSpacePhotosMutation.isPending}
              loading={updateSpacePhotosMutation.isPending}
