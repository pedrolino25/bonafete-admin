...
export default function SpaceGeneralConfigurationSection({
  ...
  const updateSpaceGeneralConfigurationMutation = useMutation({
    mutationFn: updateSpaceGeneralConfiguration,
    onSuccess: () => {
      refetch?.()
      toast({
        variant: 'success',
  ...
  const onSubmit = (values: SpaceGeneralConfigurationFormType) => {
    updateSpaceGeneralConfigurationMutation.mutate({
        ...
        <div className="flex justify-between items-center gap-4 max-sm:justify-end max-sm:items-start max-sm:pt-4 max-sm:w-full">
          <Button
            ...
            disabled={
              !isValid ||
              !isDirty ||
              updateSpaceGeneralConfigurationMutation.isPending
            }
            loading={updateSpaceGeneralConfigurationMutation.isPending}
