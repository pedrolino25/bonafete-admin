...
import { SpaceInfo, SpacePrice, updateSpaceRental } from '@/services/api/spaces'
...
export default function SpaceRentalSection({
  ...
  const updateSpaceRentalMutation = useMutation({
    mutationFn: updateSpaceRental,
    onSuccess: () => {
      refetch?.()
      toast({
        variant: 'success',
  ...
  const onSubmit = (values: SpaceRentalFormType) => {
    updateSpaceRentalMutation.mutate({
      ...
      <div className="w-full border-b px-6 max-sm:px-4 pb-4 flex justify-between items-center max-sm:flex-col">
        ...
        <div className="flex justify-between items-center gap-4 max-sm:justify-end max-sm:items-start max-sm:pt-4 max-sm:w-full">
          <Button
            type="submit"
            disabled={
              !isValid || !isDirty || updateSpaceRentalMutation.isPending
            }
            loading={updateSpaceRentalMutation.isPending}
