...
import { SpaceInfo, updateSpaceRules } from '@/services/api/spaces'
...
export default function SpaceRulesSection({
  ...
  const updateSpaceRulesMutation = useMutation({
    mutationFn: updateSpaceRules,
    onSuccess: () => {
      refetch?.()
      toast({
        variant: 'success',
  ...
  const onSubmit = (values: SpaceRulesFormType) => {
    updateSpaceRulesMutation.mutate({
        ...
        <div className="flex justify-between items-center gap-4 max-sm:justify-end max-sm:items-start max-sm:pt-4 max-sm:w-full">
          ...
          <Button
            ...
            disabled={
              !isValid ||
              (completed && !isDirty) ||
              updateSpaceRulesMutation.isPending
            }
            loading={updateSpaceRulesMutation.isPending}
