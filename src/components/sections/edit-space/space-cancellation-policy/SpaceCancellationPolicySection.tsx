...
export default function SpaceCancellationPolicySection({
  ...
  const updateSpaceCancellationPolicyMutation = useMutation({
    mutationFn: updateSpaceCancellationPolicy,
    onSuccess: () => {
      refetch?.()
      toast({
        variant: 'success',
  ...
  const onSubmit = (values: CancellationPolicyFormType) => {
    updateSpaceCancellationPolicyMutation.mutate({
        ...
        <div className="flex justify-between items-center gap-4 max-sm:justify-end max-sm:items-start max-sm:pt-4 max-sm:w-full">
          ...
          <Button
            ...
            disabled={
              !isValid ||
              (completed && !isDirty) ||
              updateSpaceCancellationPolicyMutation.isPending
            }
            loading={updateSpaceCancellationPolicyMutation.isPending}
