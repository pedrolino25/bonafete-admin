...
interface PostalCodesListSectionProps {
  data: PostalCodesListItemResponse[] | undefined
  isPending: boolean
  refresh: () => void
}

export default function PostalCodesListSection({
  data,
  isPending,
  refresh,
}: PostalCodesListSectionProps) {
  const t = useTranslations()
  const router = useRouter()
  const { toast } = useToast()

  const columns: ColumnDef<PostalCodesListItemResponse>[] = [
    {
      accessorKey: 'id',
      id: 'id',
      header: () => {
  ...
  const [selected, setSelected] = useState<PostalCodesListItemResponse>()
  ...
  const table = useReactTable({
    ...
    meta: {
      editData: (values: PostalCodesListItemResponse) =>
        router.push(`/reference-data/postal-codes/edit?id=${values.id}`),
      deleteData: (values: PostalCodesListItemResponse) => {
        setSelected(values)
        setOpenRemove(true)
      },
    },
  ...
  const deletePostalCodeMutation = useMutation({
        ...
        <DataTable.HeaderActionsContainer className="pl-4">
          <div className="w-full flex items-center gap-3">
            ...
            <Button
              ...
              onClick={() => router.push('/reference-data/postal-codes/create')}
            >
              {t('table.create')}
            </Button>
        ...
        <DialogContent className="sm:max-w-[425px]">
          ...
          <DialogFooter>
            ...
            <Button
              color="destructive"
              onClick={() => {
                deletePostalCodeMutation.mutate({
