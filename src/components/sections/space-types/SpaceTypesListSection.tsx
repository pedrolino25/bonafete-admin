...
interface SpaceTypesListSectionProps {
  data: SpaceTypeListItemResponse[] | undefined
  isPending: boolean
  refresh: () => void
}

export default function SpaceTypesListSection({
  data,
  isPending,
  refresh,
}: SpaceTypesListSectionProps) {
  const t = useTranslations()
  const router = useRouter()
  const { toast } = useToast()

  const columns: ColumnDef<SpaceTypeListItemResponse>[] = [
    {
      accessorKey: 'id',
      id: 'id',
      header: () => {
  ...
  const [selected, setSelected] = useState<SpaceTypeListItemResponse>()
  ...
  const table = useReactTable({
    ...
    meta: {
      editData: (values: SpaceTypeListItemResponse) =>
        router.push(`/reference-data/space-types/edit?id=${values.id}`),
      deleteData: (values: SpaceTypeListItemResponse) => {
        setSelected(values)
        setOpenRemove(true)
      },
    },
  ...
  const deleteSpaceTypeMutation = useMutation({
        ...
        <DataTable.HeaderActionsContainer className="pl-4">
          <div className="w-full flex items-center gap-3">
            ...
            <Button
              ...
              onClick={() => router.push('/reference-data/space-types/create')}
            >
              {t('table.create')}
            </Button>
        ...
        <DialogContent className="sm:max-w-[425px] max-sm:max-w-100svw">
          ...
          <DialogFooter>
            ...
            <Button
              color="destructive"
              onClick={() => {
                deleteSpaceTypeMutation.mutate({
