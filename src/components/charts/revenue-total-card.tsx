...
interface RevenueTotalCardProps {
  data: { month: string; revenue: string }[]
}

export default function RevenueTotalCard({ data }: RevenueTotalCardProps) {
  const t = useTranslations()

  const chartData = data?.map((item) => {
    return {
      month: item.month,
      revenue: parseInt(item.revenue) / 100,
    }
  })
  ...
  const total = chartData?.reduce((total, item) => total + item.revenue, 0)
